...
import csv
import sys
sys.path.append('/data2/xdw/NTS-Net/')
#import scipy.misc
import os
import torch.utils.data
#from PIL import Image
#from torchvision import transforms
from config import INPUT_SIZE, use_attribute, need_attributes_idx,use_uniform_mean
import cv2
import torch
...
class tooth_dataset_train():
    def __init__(self, anno_path, is_train=True):
        ...
        r = csv.reader(open(self.anno_path))
        self.num_teeth = 0 
        for line in r:
            train_flag = line[2]
            if not train_flag=='train':
                continue
            tooth_id = str(line[3])
            tooth_path = line[1]
            cur_attri = []
    ...
    def __getitem__(self, index):
        #use_attribute = '12'
        if index <= self.index_11:
            cur_use_attri = '11'  
            index = index - 1      
        elif self.index_11 < index <=self.index_12:
            cur_use_attri = '12'
            index = index - (self.index_11+1)
        elif self.index_12 < index <=self.index_21:
            cur_use_attri = '21'
            index = index - (self.index_12+1)
        elif self.index_21 < index <=self.index_22:
            cur_use_attri = '22'
